...
| 步骤 | 工作内容 | 预估 |
|---|---|---|
| 1 | `token.ts`: Token/TokenType 定义 | 小 |
| 2 | `lexer.ts`: 完整词法分析器 | 中 |
| 3 | `ast.ts`: AST 节点类型定义 | 小 |
| 4 | `parser.ts`: 递归下降解析器(含容错) | 大 |
| 5 | `document-cache.ts`: 文档缓存 | 小 |
| 6 | 改造 providers 使用 AST | 中 |
| 7 | `analyzer.ts`: 符号表 + 基本语义检查 | 大 |
| 8 | `preprocessor.ts`: 基本预处理 | 中 |

建议执行顺序: 1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7 -> 8

步骤 1-5 完成后即可替代现有的正则方案; 步骤 6 改造 provider; 步骤 7-8 渐进增强.
